...
public class AlacarteSubscriptionListAdapterUpdate  extends RecyclerView.Adapter<AlacarteSubscriptionListAdapterUpdate.MyViewHolder> {

    Context context;
    public  String packageId, pkgName, pkgPrice, taxPer, taxAmnt;

    List<AlacarteModel.AlacarteDTO> alacaretes;
    List<AlacarteModel.AlacarteDTO> temp;
    AdapterCallbackForUpdate adapterCallback;

    public AlacarteSubscriptionListAdapterUpdate(Context context, List<AlacarteModel.AlacarteDTO> alacaretes, List<AlacarteModel.AlacarteDTO> temp, AdapterCallbackForUpdate adapterCallback) {
    ...
    @NonNull
    @Override
    public AlacarteSubscriptionListAdapterUpdate.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_channel_list, parent, false);
        return new AlacarteSubscriptionListAdapterUpdate.MyViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(@NonNull final AlacarteSubscriptionListAdapterUpdate.MyViewHolder holder, @SuppressLint("RecyclerView") final int position) {
        ...
        holder.rb_insert_delete_channel.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (alacaretes.get(position).isIs_select()) {
                    //remove
                    adapterCallback.onClickCallback(holder.ll, position, "Remove",alacaretes.get(position));

                } else {
                    //insert
                    adapterCallback.onClickCallback(holder.ll, position, "Add",alacaretes.get(position));
