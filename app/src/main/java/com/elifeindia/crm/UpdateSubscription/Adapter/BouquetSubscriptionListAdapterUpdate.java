...
public class BouquetSubscriptionListAdapterUpdate  extends RecyclerView.Adapter<BouquetSubscriptionListAdapterUpdate.MyViewHolder> {
    ...
    @NonNull
    @Override
    public BouquetSubscriptionListAdapterUpdate.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_channel_list, parent, false);
        return new BouquetSubscriptionListAdapterUpdate.MyViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(@NonNull final BouquetSubscriptionListAdapterUpdate.MyViewHolder holder, @SuppressLint("RecyclerView") final int position) {
        ...
        holder.rb_insert_delete_channel.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(bouquets.get(position).isIs_select()){
                    //remove
                    adapterCallback.onClickCallback(holder.ll, position, "Remove",bouquets.get(position));

                }else{
                    //insert
                    adapterCallback.onClickCallback(holder.ll, position, "Add",bouquets.get(position));
