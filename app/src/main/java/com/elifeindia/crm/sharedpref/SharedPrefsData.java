package com.elifeindia.crm.sharedpref;
...
public class SharedPrefsData {
    ...
    private static final String POULTRY_DATA = "elifecrm";
    ...
    public static void putFloat(Context context, Float key, float value, String pref) {
        if (context == null || key == null) {
            return;
        }
        if (pref == null || pref.isEmpty()) {
            PreferenceManager.getDefaultSharedPreferences(context).edit().putFloat(String.valueOf(key), value).apply();
        } else {
            context.getSharedPreferences(pref, Context.MODE_PRIVATE).edit().putFloat(String.valueOf(key), value).apply();
        }
    }

    public static float getFloat(Context context, String key, String pref) {
        if (context == null || key == null) {
            return 0;
        }
        if (pref == null || pref.isEmpty()) {
            ...
            return context.getSharedPreferences(pref, Context.MODE_PRIVATE).getFloat(key, 0);
        }
